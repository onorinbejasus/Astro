...
Projection = (function() {
  ...
  Projection.prototype.unproject = function(xsize, ysize) {
    var dec, decpole, dtor, i, j, l, lat, latpole, long, lonpole, lp, m, mat, mp, n, np, phi, r, r11, r12, r13, r21, r22, r23, r31, r32, r33, ra, rapole, rtod, theta, u, v, x, xpix, y, ypix, _i, _j, _k, _l, _m, _n, _o, _p, _q, _r, _results, _results1, _results2, _results3, _results4, _results5, _results6, _results7, _results8, _results9, _s, _t;
    ...
    u = (function() {
      _results4 = [];
      for (var _m = 0; 0 <= xsize ? _m <= xsize : _m >= xsize; 0 <= xsize ? _m++ : _m--){ _results4.push(_m); }
      return _results4;
    }).apply(this);
    v = (function() {
      _results5 = [];
    ...
    for (i = _o = 0; 0 <= xsize ? _o <= xsize : _o >= xsize; i = 0 <= xsize ? ++_o : --_o) {
      ra[i] = (function() {
        _results6 = [];
        for (var _p = 0; 0 <= ysize ? _p <= ysize : _p >= ysize; 0 <= ysize ? _p++ : _p--){ _results6.push(_p); }
        return _results6;
      }).apply(this);
      dec[i] = (function() {
        _results7 = [];
        for (var _q = 0; 0 <= ysize ? _q <= ysize : _q >= ysize; 0 <= ysize ? _q++ : _q--){ _results7.push(_q); }
        return _results7;
      }).apply(this);
      u[i] = (function() {
        _results8 = [];
        for (var _r = 0; 0 <= ysize ? _r <= ysize : _r >= ysize; 0 <= ysize ? _r++ : _r--){ _results8.push(_r); }
        return _results8;
      }).apply(this);
      v[i] = (function() {
        _results9 = [];
        for (var _s = 0; 0 <= ysize ? _s <= ysize : _s >= ysize; 0 <= ysize ? _s++ : _s--){ _results9.push(_s); }
        return _results9;
      }).apply(this);
      for (j = _t = 0; 0 <= ysize ? _t <= ysize : _t >= ysize; j = 0 <= ysize ? ++_t : --_t) {
        ...
        phi = 0.0;
        theta = 90.0 * dtor;
        lonpole = this.parameters.crval1 > 0.0 ? 0.0 : 180.0 * dtor;
        latpole = 90.0 * dtor;
