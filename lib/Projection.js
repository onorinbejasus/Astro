...
Projection = (function() {
  ...
  Projection.prototype.init = function(image, fits, Tile, survey) {
    ...
    if (survey === "LSST" || survey === "FIRST") {
      ...
      $.getJSON("./lib/webgl/imageHeader.php?url=" + image + "&survey=" + survey + "&type=JPEG", function(data) {
        return $.each(data, function(key, val) {
          if (key === "CRVAL_1") _this.parameters.crval1 = val;
          if (key === "CRVAL_2") _this.parameters.crval2 = val;
          if (key === "CRPIX_1") _this.parameters.crpix1 = val;
          if (key === "CRPIX_2") _this.parameters.crpix2 = val;
          if (key === "CD1_1") _this.parameters.cd11 = val;
          if (key === "CD1_2") _this.parameters.cd12 = val;
          if (key === "CD2_1") _this.parameters.cd21 = val;
          if (key === "CD2_2") _this.parameters.cd22 = val;
          if (key === "CTYPE_1") _this.parameters.ctype1 = val;
          if (key === "CTYPE_2") _this.parameters.ctype2 = val;
          if (key === "CDELT_1") _this.parameters.cdelt1 = val;
          if (key === "CDELT_2") return _this.parameters.cdelt2 = val;
        });
      });
      $.ajaxSetup({
        'async': true
      ...
      $.getJSON("./lib/webgl/imageHeader.php?url=" + fits + "&type=TEXT&survey=SDSS", function(data) {
        return $.each(data, function(key, val) {
          if (key === "CRVAL_1") _this.parameters.crval1 = val;
          if (key === "CRVAL_2") _this.parameters.crval2 = val;
          if (key === "CRPIX_1") _this.parameters.crpix1 = val;
          ...
          if (key === "CD2_1") _this.parameters.cd21 = val;
          if (key === "CD2_2") _this.parameters.cd22 = val;
          if (key === "CTYPE1") _this.parameters.ctype1 = val;
          if (key === "CTYPE2") return _this.parameters.ctype2 = val;
        });
      });
      $.ajaxSetup({
        'async': true
  ...
  Projection.prototype.unprojectSIN = function(xsize, ysize) {
    var coords, crval, dec, dtor, i, index, indices, j, lat, long, r, ra, rtod, tmp, x, xpix, y, ypix, _i, _j, _results, _results2;
    ...
    ypix = (function() {
      _results2 = [];
      for (var _j = 1; 1 <= ysize ? _j <= ysize : _j >= ysize; 1 <= ysize ? _j++ : _j--){ _results2.push(_j); }
      return _results2;
    }).apply(this);
    ra = [0, 1, 2, 3];
    dec = [0, 1, 2, 3];
    ...
    */
    for (index = 0; index <= 3; index++) {
      ...
      if (this.parameters.ctype1 === "DEC--SIN") {
        ...
        x = y;
        y = tmp;
        if (index === 0) crval = this.Math.rotate(crval);
      }
      /*			
    ...
    */
    for (index = 0; index <= 3; index++) {
      ...
      */
      ...
      r = Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2));
      if (r > 0.0) lat = Math.atan((180.0 / Math.PI) / r);
