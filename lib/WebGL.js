(function() {
  ...
  WebGL = (function() {
    ...
    function WebGL(options) {
      ...
      this.initGL = __bind(this.initGL, this);      this.canvas = options.canvas != null ? options.canvas : (this.canvas = document.createElement("canvas"), this.canvas.width = options.clientWidth, this.canvas.height = options.clientHeight, this.canvas.style.backgroundColor = "rgb(0,0,0)", options.appendChild(this.canvas));
      this.initGL();
      this.initShaders();
    ...
    */
    ...
    WebGL.prototype.initGL = function() {
      try {
        ...
        if (!this.gl) return alert("Could not initialize WebGL, sorry :-(");
      }
    };
