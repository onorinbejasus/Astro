...
SkyView = (function(_super) {
  ...
  SkyView.prototype.mousePress = function(key) {
    ...
    near = this.Math.norm(this.Math.unProj(key.x, key.y, 0, matrices[0], matrices[1], matrices[2]));
    far = this.Math.norm(this.Math.unProj(key.x, key.y, 1, matrices[0], matrices[1], matrices[2]));
    dir = this.Math.norm(this.Math.subtract(far, near));
    tri = this.HTM.getTriangles();
    for (_i = 0, _len = tri.length; _i < _len; _i++) {
      triangle = tri[_i];
      console.log(this.Math.intersectTri([0, 0, 0], dir, triangle, near, far));
    }
  };
