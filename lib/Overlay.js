...
Overlay = (function() {
  ...
  Overlay.prototype.createSDSSOverlay = function() {
    ...
    $.getJSON("./lib/webgl/SDSSFieldQuery.php?ra=" + ra + "&dec=" + dec + "&radius=			" + radius + "&zoom=00", function(data) {
      return $.each(data, function(key, val) {
        var fits, fitsFile;
        if (key % 2 === 0) {
          fitsFile = data[key + 1];
          fits = fitsFile.split(".")[0].concat(".").concat(fitsFile.split(".")[1]);
          return _this.tiles.push(new Tile(_this.SkyView.gl, _this.SkyView.Math, "SDSS", "sky", "" + val, "/afs/cs.pitt.edu/projects/admt/web/sites/astro/headers/" + fits, null));
        }
      });
    });
    return $.ajaxSetup({
